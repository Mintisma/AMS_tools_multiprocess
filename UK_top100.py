def get_top100(index, num):
    ...
    def price_get(sku):
        try:
            float(sku.parent.select('span.a-size-base')[0].get_text().replace('£', '').replace(',', '.'))
        except Exception as err:
            r = 0
        else:
            r = float(sku.parent.select('span.a-size-base')[0].get_text().replace('£', '').replace(',', '.'))
        return r
    ...
    url = 'https://www.amazon.co.uk/Best-Sellers-Lighting-Lamp-Bases/zgbs/lighting/{}?pg={}'.format(index, num)
