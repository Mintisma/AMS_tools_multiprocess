...
def get_top100(index, num):
    def price_get(sku):
        try:
            float(sku.parent.select('span.a-size-base')[0].get_text().replace('$', ''))
        except Exception as err:
            r = 0
        else:
            r = float(sku.parent.select('span.a-size-base')[0].get_text().replace('$', ''))
        return r

    def star_get(sku):
        try:
            float(sku.parent.select('a.a-link-normal')[1].get('title').split()[0])
        except Exception as err:
            r = 0
        else:
            r = float(sku.parent.select('a.a-link-normal')[1].get('title').split()[0])
        return r
